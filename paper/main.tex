...
\begin{figure}
  ...
  \caption{Chamber-structure family champions from the supplemental chamber campaign. The axes keep the paper's central productivity/compromise comparison visible; malapportionment remains in Table~\ref{tab:chamber-family-champions} because the champion set has little variation on that metric. The figure is a structural sensitivity view, not a replacement for the main scenario comparison.}
...
\begin{figure}[H]
  ...
  \caption{Value-profile matrix for selected systems. Rows are sorted by the same overall directional score used in Table~\ref{tab:scenario-averages}. Green cells are more favorable and red cells are less favorable under the stated metric interpretation; the red row label marks the stylized U.S.-like benchmark. This figure should be read as a profile comparison, not as a proof of one best system.}
...
\begin{figure}[H]
  \centering
  \resizebox{0.66\linewidth}{!}{\input{figures/timeline_contention}}
