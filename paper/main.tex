...
Because this paper's institutional question gives special normative importance to negotiated moderation, the main representative comparison also reports an explicit compromise-priority sensitivity score: 0.30 compromise, 0.20 productivity, 0.20 risk control, 0.15 administrative feasibility, and 0.15 public-mandate legitimacy. This is not an estimated social-welfare function. It is a transparent value profile used to test whether the qualitative ordering changes when compromise receives direct priority.
...
\begin{figure}
  ...
  \caption{Chamber-structure family champions from the supplemental chamber campaign, zoomed to the observed champion range so clustered family labels remain readable. The axes keep the paper's central productivity/compromise comparison visible; malapportionment remains in Table~\ref{tab:chamber-family-champions} because the champion set has little variation on that metric. The figure is a structural sensitivity view, not a replacement for the main scenario comparison.}
  \label{fig:chamber-productivity-compromise}
  \Description{Scatter plot of chamber family champion productivity and compromise, with the conventional benchmark marked in red.}
\end{figure}
...
Table~\ref{tab:scenario-averages} compares representative family examples across broad and adversarial synthetic cases, sorted by the compromise-priority sensitivity score; the appendix reports the full table with directional and seed diagnostics. Table~\ref{tab:pareto-front} shows the narrow productivity/compromise/risk frontier. It is not a recommendation because it excludes administrative feasibility and compresses risk into one objective. Table~\ref{tab:mechanism-diagnostics} adds ablation and manipulation-stress diagnostics, while Table~\ref{tab:empirical-bridge} separates the limited empirical bridge from the synthetic mechanism findings.

The main results are tradeoffs, not rankings. Under the compromise-priority sensitivity profile, pairwise alternatives and amendment tournaments move to the top because they preserve substantial throughput while producing the highest compromise and strong risk control. The portfolio and expanded-portfolio hybrids remain close behind, while default enactment falls because high productivity no longer offsets weak compromise and weak public-mandate performance. Backward-to-floor systems sharpen the interpretation. Petition and open-calendar variants preserve access while routing risk to deliberation; discharge and proportional-committee variants expose committee-design effects; anti-capture access filters before floor consideration; and amendment tournaments test content control. The portfolio hybrid is a design hypothesis, not a winner. The stylized U.S.-like benchmark has low productivity but stronger compromise, risk, and mandate diagnostics than its directional-score rank suggests; Table~\ref{tab:scenario-averages}'s yield and blockage-dependence columns distinguish gatekeeping success from policy production.
...
\begin{figure}[!htbp]
  ...
  \begin{minipage}[t]{0.49\textwidth}
    ...
    \centerline{\small (a) Representative-risk burden}
  \end{minipage}\hfill
  \begin{minipage}[t]{0.49\textwidth}
    \centering
  ...
  \end{minipage}
  \caption{Two productivity tradeoff views for the representative systems in Table~\ref{tab:scenario-averages}, rendered side by side with matching 0--1 axes. Panel (a) plots representative-risk burden, defined as \(1-\)risk control; lower is better. Panel (b) plots compromise under weighted party-system sensitivity cases; higher is better. The stylized U.S.-like benchmark is red and marked with a larger square.}
...
\begin{figure}[H]
  ...
  \caption{Value-profile matrix for selected systems. Rows are sorted by the compromise-priority sensitivity score used in Table~\ref{tab:scenario-averages}. Green cells are more favorable and red cells are less favorable under the stated metric interpretation; the red row label marks the stylized U.S.-like benchmark. This figure should be read as a profile comparison, not as a proof of one best system.}
  \label{fig:directional-scoreboard}
  \Description{Matrix comparing compromise-priority score, productivity, compromise, public-mandate legitimacy, risk control, and administrative feasibility for selected institutional scenarios.}
\end{figure}

\begin{figure}[H]
  \centering
  \resizebox{0.78\linewidth}{!}{\input{figures/timeline_contention}}
  \caption{Selected rising-contention stress paths from the main comparison campaign. The index combines gridlock, compromise loss, and weak public-mandate passage. Lower is better. This is a parameterized stress path, not an endogenous causal model of polarization.}
