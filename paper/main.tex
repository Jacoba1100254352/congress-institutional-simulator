...
The contribution is methodological. The simulator separates productivity from compromise, public alignment, weak public-mandate passage, proposer advantage, concentrated harm, lobbying capture, administrative cost, and correction over time. A single comparison campaign generates all reported tables and figures from one CSV artifact, with assumption-sensitivity bands, seed checks, mechanism ablations, and manipulation-stress diagnostics. The current experiment is breadth-first: default enactment remains one burden-shifting stress test, while the main comparison spans conventional, deliberative, anti-capture, agenda-scarcity, policy-tournament, harm-protection, and review-oriented families. The paper argues for mechanism search: legislative designs should be compared as portfolios of incentives and safeguards, not as isolated passage thresholds.
\end{abstract}
...
Table~\ref{tab:scenario-averages} compares representative family examples across broad and adversarial assumption cases, while the appendix reports the full scenario table. Table~\ref{tab:pareto-front} makes the narrow productivity/compromise/risk frontier explicit. The frontier should not be read as a recommendation: it excludes administrative feasibility and treats generated risk control as a single combined objective. Table~\ref{tab:mechanism-diagnostics} adds a second diagnostic layer: ablations ask which components appear to do work, while stress rows ask how promising mechanisms degrade under manipulation pressure.
...
\begin{figure}[H]
  ...
  \caption{Productivity versus representative-risk burden for the representative systems in Table~\ref{tab:scenario-averages}. The vertical axis is \(1-\)risk control, combining chamber low-support passage, weak public-mandate passage, minority harm, lobbying capture, public-preference distortion, proposer gain, concentrated-harm passage, and policy shift. The stylized U.S.-like benchmark is red and marked with a larger square. Productive systems in the lower-right region are preferable under these two metrics.}
...
\begin{figure}[H]
  ...
  \caption{Compromise versus productivity for the representative systems in Table~\ref{tab:scenario-averages}, using weighted party-system sensitivity cases from the same main comparison campaign. The plot makes the main target visible: systems should move right without collapsing downward.}
...
\end{verbatim}
The paper build regenerates the main comparison campaign before drawing tables or figures, so the PDF is tied to the tracked campaign artifact rather than to stale CSV files. The seed-robustness target writes a multi-seed report for every paper scenario, and Table~\ref{tab:scenario-averages} includes a compact seed half-range column for the directional diagnostic alongside assumption-sensitivity bands across campaign cases. The ablation and manipulation-stress targets write diagnostic reports for component removal and strategic attack cases. The empirical-bridge target maps optional raw-data summaries to the calibration screen, making clear which real-world signals are still missing. The family-screen target writes a supplemental breadth-catalog comparison under a fixed selection rule, and catalog-breadth records which historical default-pass variants are archived outside that screen. The anonymous supplement target packages source, paper files, generated reports, and review PDFs while excluding repository metadata and local artifacts. A public repository URL will be added after review if anonymity rules permit; a separate public provenance manifest can be generated after review with \texttt{make public-provenance}.
