...
The calibration runner executes conventional scenarios and writes CSV and Markdown pass/fail reports under \texttt{reports/}. The current calibration pass is an empirical screen, not an automatic parameter fitter. Its benchmark ranges are intentionally broad and several are abstract proxies, so passing the screen should be read as minimum plausibility rather than validation. The empirical bridge currently includes six documented raw validation samples: Congress.gov bill progression, Voteview roll calls, Senate LDA lobbying disclosures, and Congress.gov-derived topic, sponsor, and committee summaries. These samples support bill-attrition, floor-load, committee-report, coalition-size, party-unity, sponsor-concentration, topic-throughput, and lobbying-concentration checks. Optional API fixture samples are stored separately from raw validation inputs; they test adapter shapes but are not used as validation evidence. The calibration screen's purpose is to keep ordinary baselines within explicit plausible ranges before interpreting counterfactual mechanisms.
...
Structured mediation moves risky proposals toward weighted institutional targets. Multi-round mediation models repeated amendments, costs, concession limits, poison-pill risk, capacity pressure, public-mandate improvement against a no-amendment counterfactual, proposer-advantage reduction, and compensation for concentrated harm.
...
\begin{itemize}
  ...
  \item \texttt{make empirical-bridge}: map raw summaries to simulator proxies.
  \item \texttt{make validation-gap-report}: generate the paper-facing validation-boundary report. Adapter fixtures live separately and do not count as raw validation data.
  ...
  \item \texttt{make build-core-raw-validation}: rebuild the broader Voteview, Congress.gov-derived, and Senate LDA raw bridge samples when network access and any required API key are available.
