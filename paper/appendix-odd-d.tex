...
An institutional process is a composable module. Processes can screen proposal access, apply leadership agenda control, modify public signals, spend lobbying or attention resources, route bills to procedural tracks, mediate amendments, evaluate affected-group harm, select among alternatives, conduct chamber votes, revise through conference committees, apply vetoes, apply judicial review, route through citizen initiatives, register enacted laws, or review laws later. The final decision still resolves to a yes-or-no outcome, enactment or rejection, and a post-bill status quo.
...
The main submitted evidence comes from the v21-paper campaign. That campaign joins broad assumption cases, adversarial proposal-generator cases, party-system sensitivity cases, and a stylized rising-contention timeline in one artifact. It is now breadth-first: it includes the stylized U.S.-like benchmark, conventional affirmative baselines, leadership agenda control, committee-first regular order, cloture with conference and judicial review, parliamentary-style coalition confidence, direct citizen initiatives, non-default policy tournaments, citizen assembly review, public-interest screening, agenda lotteries, quadratic attention budgets, proposal bonds, harm-weighted thresholds, compensation amendments, package bargaining, public objection, law-registry review, anti-capture safeguards, adaptive risk routing, norm erosion, and a small default-enactment stress-test family. Adversarial generator cases test high-benefit extreme reform, popular harmful bills, moderate capture, delayed-benefit reform, concentrated rights-like harm, and anti-lobbying backlash. Earlier v17--v20 campaigns remain historical development artifacts, including deeper default-pass sweeps, but are not mixed into the main paper evidence base.
...
Run tests with \texttt{make test}. Run the current campaign with \texttt{make campaign} or \texttt{make campaign-v21-paper}. Run empirical benchmark screening with \texttt{make calibrate}. Run multi-seed checks for all paper scenarios with \texttt{make seed-robustness-check}. Run raw-data readiness checks with \texttt{make validation-readiness}, and compute raw-data summaries from any local optional inputs with \texttt{make empirical-validation}. Run the breadth-catalog family champion screen with \texttt{make family-champions} and document catalog composition with \texttt{make catalog-breadth}. Build the paper and appendix PDFs with \texttt{make paper}. Build a double-blind supplement with \texttt{make supplement-anonymous}. After review, generate a non-anonymous provenance manifest with \texttt{make public-provenance}. The review package includes generated campaign reports and PDFs; public repository identifying details are withheld during double-blind review and can be restored after review if permitted.
