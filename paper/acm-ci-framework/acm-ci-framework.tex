...
The main paper uses a tradeoff figure plus compact tables to carry the results argument; detailed numeric tables, Pareto rows, ablation deltas, manipulation-stress losses, and separate PAIR/AMT rows are moved to the appendix.

Three substantive patterns appear. First, high-throughput systems can also produce low-public-support enactments, so productivity needs to be read beside support and risk diagnostics. Second, content-selection stages have the largest effect under the implemented generator because they alter which alternatives enter final aggregation rather than only changing the yes/no threshold. Pairwise alternatives and amendment tournaments are combined as one content-selection family in the main displays; their separate appendix rows are nearly identical and should not be read as independent evidence of robustness. Third, portfolio systems reduce some risks but add administrative load. The stylized U.S.-like benchmark has low productivity but stronger revision moderation, risk, and support diagnostics than its directional profile suggests.

The empirical flow sanity checks do not validate the welfare, revision-moderation, harm, capture, or public-support metrics used in the main comparison; they only screen whether several observable legislative-flow proxies fall in plausible ranges.
