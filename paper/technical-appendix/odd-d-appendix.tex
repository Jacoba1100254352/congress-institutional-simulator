...
\begin{abstract}
This appendix documents the congressional institutional simulator using the ODD and ODD+D model-description conventions \citep{Grimm2010,Muller2013}. It describes entities, state variables, scheduling, adaptation, input data, submodels, outputs, empirical boundary checks, assumptions, and reproducibility commands for the simulator used in \emph{A Reproducible Simulation Framework for Comparing Legislative Mechanism Bundles}.
\end{abstract}
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{p{0.43\linewidth}p{0.45\linewidth}}
    ...
    What are the selected main-campaign results? & Table~\ref{tab:scenario-averages}; full CSV in \path{reports/simulation-campaign-v21-paper.csv} \\
...
The House mechanism panel narrows the unit to every H.R. measure in the complete 116th--118th-Congress censuses and joins direct H.R. and H. Res. actions to literal Table~1a entries in the official House Rules Committee activity surveys \citep{HouseRulesSurvey116,HouseRulesSurvey117,HouseRulesSurvey118}. It retains 29,335 source-pinned H.R. rows and 458 rule-grant rows. Among 2,176 floor-considered bills, the source classifier assigns exactly one of four observed routes: 282 special-rule-only, 1,792 suspension-only, 42 mixed special-rule and suspension, and 60 other detected routes. Every H.R. row retains source-record and canonical-action hashes, and every rule row retains its direct resolution source. The 117th survey narrative reports two more structured and two more closed rules than literal Table~1a; the table is authoritative for row construction and the discrepancy remains in metadata. Stage durations are computed only for linear action histories, leaving 117 non-linear intervals blank rather than imposing an order.

The recorded route-calibration protocol follows source audit, but its asserted pre-fit timing has no separate commit in the available Git history. The artifact retains the original specification and a provenance amendment documenting this limit. Because the 118th source distribution was already known, this is a no-refit temporal test rather than an outcome-blind preregistration. Development pools all 18,771 H.R. measures from the 116th and 117th Congresses. The fixed grid crosses five calendar-priority thresholds with seventeen special-rule and seventeen suspension thresholds, yielding 1,445 candidates. Each candidate uses 20 seeds, 12 runs per seed, and 14,400 simulated bills; the unchanged test uses 30 fresh seeds, 20 runs per seed, and 36,000 bills. Selection minimizes tolerance-standardized error in floor rate and four route shares. The selected calendar, special-rule, and suspension thresholds are 0.680, 0.475, and 0.750, with loss 4.248261 and maximum standardized error 1.712057; 16 of 20 leave-one-seed-out panels reselect the triple. The suspension threshold is the upper grid boundary. The artifact retains all 20 selected-candidate development seeds and 30 test seeds, with standard deviations and standard errors describing conditional Monte Carlo variability. These summaries do not replace the pooled acceptance metrics.

The no-refit 118th test passes committee advancement (0.107528 simulated versus 0.113783 observed), floor consideration (0.062806 versus 0.063991), and restrictive special-rule share (1.000000 versus 0.992593). Four-route total variation is 0.110470, exceeding its locked 0.100 tolerance by 0.010470. The observed special-only, suspension-only, mixed, and other shares are 0.187870, 0.804734, 0.005917, and 0.001479; simulated shares are 0.077399, 0.812915, 0.057054, and 0.052632. Thus the unchanged model misses the later shift toward special-rule-only consideration and allocates excess mass to mixed and other routes. The candidate grid is not expanded and no threshold is retuned. Route indicators are deterministic diagnostics assigned after calendar and status-quo gates; they do not change voting or enactment outcomes. The retained 3-of-4 gate failure therefore falsifies this aggregate route construction without overwriting the separate lifecycle calibration or supporting a causal account of House agenda control.

A denominator-aligned executive-action diagnostic prevents aggregate fit from hiding a mechanism error. One panel parses 126,760 H.R./S. records from 22 pinned 108th--118th-Congress GovInfo archives and retains 4,021 presidential decisions; a separate 22-archive H.J.Res./S.J.Res. panel parses 2,031 records and retains 187 decisions. Each Congress and measure class satisfies presentments equal enactments plus vetoes minus overrides. The bill class contains 21 vetoes and six successful overrides, while the joint-resolution class contains 26 vetoes and no overrides. The combined conditional veto rate is 0.011169 (95\% Wilson interval [0.008410, 0.014820]). The frozen selected simulator panel produces 647 vetoes among 2,621 executive decisions, 0.246852 [0.230724, 0.263722], or 22.101 times the combined empirical rate; the intervals do not overlap. The exact veto sets, dates, kinds, and override outcomes match official Senate histories; president and chamber-control context follows the official House history. A paired final-approval panel contributes 8,416 chamber rows for the 4,208 presentments: 1,685 official final roll calls and 6,731 voice-vote, unanimous-consent, or other nonrecorded final approvals. Earlier recorded votes are never substituted for later nonrecorded approvals.

The presidential-choice study was fixed after source construction and descriptive source audit but before model implementation or fitting. The primary L2-penalized logistic model includes six structural indicators plus minimum observed overall and opposition-party support; explicit recording indicators identify missing pathways, and support is never imputed. Every nonintercept predictor is training-standardized. The fixed penalty \(\lambda=0.16\) is equivalent to independent Gaussian \(N(0,2.5^2)\) slope regularization; it uses a Gaussian rather than the Cauchy default discussed by \citet{GelmanEtAl2008}. The primary split trains on the 108th--117th Congresses (3,921 measures; 34 vetoes) and tests the 118th (287; 13). A secondary split trains through the 115th (3,203; 24) and tests the 116th (353; 10). Log loss is primary, Brier score is secondary, and calibration-in-the-large is the mean probability minus observed rate \citep{GneitingRaftery2007}. The locked 118th gate requires lower log loss than training prevalence and absolute calibration error no greater than 0.020. The structural M1 ablation records primary log loss 0.058347 and Brier score 0.015232. The support model records 0.026977 and 0.008143 versus 0.223369 and 0.044586 for training prevalence; its calibration-in-the-large is 0.003149, so the gate passes. In the secondary check, M1 and M2 log losses are 0.062969 and 0.038738 versus 0.145942, with M2 calibration-in-the-large 0.000908. A post-fit concentration audit finds 12 of 13 primary test vetoes among only 17 joint resolutions; all 13 vetoes appear within the 16 highest predicted probabilities. Thus the result is one narrow predictive transport success, not a blinded preregistration, a causal estimate, broad out-of-regime evidence, or validation of the simulator's elevated-propensity veto stress mechanism.

The executable checker runs conventional scenarios and writes CSV and Markdown pass/fail reports under \texttt{reports/}. A pass is an empirical screen; only the calendar-capacity threshold described above is selected against the census calibration half. Benchmark ranges remain intentionally broad and several are abstract proxies, so passing should be read as minimum plausibility rather than validation. Other empirical comparison checks include a bounded sponsor aggregate with sponsor-bill metadata matches by Bioguide ID, Voteview roll-call rows carrying bounded member metadata and bill-number linkage, Senate LDA lobbying disclosures with bounded issue-policy context, bounded OpenFEC campaign-finance rows with recipient and limited issue/member/district context, a bounded Cumulative CES district-opinion aggregate \citep{CumulativeCES2025}, a bounded QoG/OWID/V-Dem comparative-institution profile \citep{BormannGolder2022,HeniszPOLCON2017,OWIDVDEMJudicial2026,OWIDVDEMLegislative2026}, SCDB merits review with bounded U.S.C.-section authority-overlap metadata, Federal Register final-rule records, and Congress.gov public-law revision text. Fourteen targeted held-out checks across nine source families pass broad ranges. Sponsor-bill metadata, district public-opinion policy-area context, comparative scenario-family anchors, LDA issue-policy context, FEC issue-sector and target-scope context, finance/lobbying source-acquisition targets, Federal Register authority and proposed-history metadata, SCDB section overlaps, topic throughput, and sampled committee summaries remain context or proxy evidence. A separate linkage audit reports all fourteen source families as linked, metadata-linked, or partially linked. Optional API fixtures test adapter shapes but are not validation evidence.

The post-fit chamber-attribution audit reopens all 29,335 pinned H.R. XML histories, verifies their record and action hashes, and retains 7,397 direct floor actions matching the original suspension/other-path phrases. One Senate motion on H.R. 4366 was counted as House suspension evidence. A direct-action-only screen changes the 118th special-only/suspension-only/mixed/other counts from 127/544/4/1 to 128/544/3/1, on the same 676-bill denominator. With unchanged simulator predictions, total variation rises to 0.111950 and remains above 0.100. The original source panel and fitted results are preserved. This is a post-fit source-quality sensitivity, not a new gate. Separate retained sources establish the indirect path: House roll call 64 adopted H. Res. 1061 under suspension by 339--85 on March 6, 2024, and the engrossed resolution deemed concurrence in the Senate amendment to H.R. 4366 with an amendment \citep{HouseRollCall2024064,HouseResolution1061EH}. This supports a mixed-history label when the indirect path is included, but does not repair the original Senate-action provenance. The direct-only screen is not a complete corrected procedural census. Source mixed routes also span a bill's history, unlike one-step score overlap in the simulator. The evidence extract, linkage reference, and two audit tables are reproduced by \texttt{make house-procedure-source-audit}.

The calendar threshold changes admission, but the special-rule and suspension thresholds only label admitted bills. They do not change the inner voting rule, impose the House's two-thirds suspension requirement, or change amendment rights. The fitted route shares therefore test a diagnostic assignment rather than the behavioral consequences of those actual procedures.

\input{figures/empirical_validation_gap_table}
...
The dense full scenario-average table is not repeated in the PDF appendix. Full numeric outputs, including separate pairwise-alternative and amendment-tournament rows, are provided in \path{reports/simulation-campaign-v21-paper.csv}; the generated LaTeX full table remains in \path{paper/figures/scenario_averages_full_table.tex} for local inspection.
...
\input{figures/sensitivity_controls_table}
\input{figures/pareto_front_table}
\input{figures/mechanism_diagnostics_table}
\input{figures/representative_champion_table}

\begin{figure}
  \centering
  \resizebox{0.9\linewidth}{!}{\input{figures/timeline_contention}}
...
\begin{itemize}
  ...
  \item \texttt{make empirical-flow-heldout}: run narrow held-out source-family benchmark checks.
  \item \texttt{make govinfo-bill-census-116}: regenerate the complete 116th-Congress H.R./S. external-cohort summary.
  \item \texttt{make govinfo-bill-census}: regenerate the full 117th-Congress H.R./S. lifecycle summary from the committed census.
  \item \texttt{make govinfo-bill-census-118}: regenerate the complete 118th-Congress H.R./S. temporal-cohort summary.
  \item \texttt{make govinfo-executive-action-panel}: verify the compact 11-Congress decision panel offline.
  \item \texttt{make legislative-lifecycle-calibration}: rerun the fixed current-Congress workflow threshold sweep.
  \item \texttt{make legislative-executive-action-diagnostic}: compare empirical presentment, veto, override, and enactment pathways with the frozen simulator panel.
  \item \texttt{make legislative-lifecycle-temporal-replication}: apply the frozen selected threshold to the 116th- and 118th-Congress rates without refitting.
  \item \texttt{make govinfo-bill-census-check}: verify all three censuses, 22 executive-panel archive pins, classifier edge cases, official veto references, cross-source agreement, calibration linkage, the 5 / 6 transport outcome, and the executive-action discrepancy.
  \item \texttt{make house-agenda-control-study}: regenerate the denominator-explicit House route, rule, and timing study from the pinned source panels.
  \item \texttt{make house-agenda-control-calibration}: rerun the locked 1,445-candidate route study and unchanged 118th-Congress test.
  \item \texttt{make house-agenda-control-calibration-check}: verify specification, source, model, build, output hashes, selected boundary triple, and the retained 3 / 4 gate failure.
  \item \texttt{make empirical-linkage-report}: audit explicit raw-data joins across source families.
  \item \texttt{make empirical-linkage-roadmap}: write join-key requirements and acceptance gates for non-fully-linked source families.
  \item \texttt{make govinfo-billstatus-linkage}: write the bounded govinfo BILLSTATUS cross-check for the cached bill-progression sample.
  \item \texttt{make court-law-linkage}: write the bounded SCDB/Federal Register U.S.C.-section authority-overlap report.
  \item \texttt{make rulemaking-authority-linkage}: write the bounded Federal Register public-law authority-search report.
  \item \texttt{make rulemaking-history-linkage}: write the bounded Federal Register proposed-rule history report for authority-matched final rules.
  \item \texttt{make bill-law-evidence-spine}: write the bounded public-law bill/action join inventory for revision-text proxies, sponsor-district public-opinion context, authority-search matches, proposed-history/comment-portal metadata, final-rule/proposed-to-final timing metadata, and court U.S.C.-section overlaps.
  \item \texttt{make district-public-opinion-policy-context}: write the bounded sponsor-district public-law bill policy-area context inventory for CES district proxy rows.
  \item \texttt{make campaign-finance-district-context}: write the bounded FEC House-candidate district-context inventory.
  \item \texttt{make campaign-finance-member-context}: write the bounded FEC candidate-to-Voteview member-context inventory.
  \item \texttt{make campaign-finance-issue-context}: write the bounded FEC transaction-label issue-context inventory.
  ...
  \item \texttt{make build-govinfo-bill-census-116-raw}: rebuild the pinned 116th-Congress H.R./S. external cohort from the official bulk archives.
  \item \texttt{make build-govinfo-bill-census-raw}: rebuild the pinned 117th-Congress H.R./S. lifecycle census from the official bulk archives.
  \item \texttt{make build-govinfo-bill-census-118-raw}: rebuild the pinned 118th-Congress H.R./S. external cohort from the official bulk archives.
  \item \texttt{make build-govinfo-billstatus-linkage-raw}: rebuild the bounded govinfo BILLSTATUS cross-check when network access is available.
  ...
  \item \texttt{make build-district-public-opinion-raw}: rebuild the bounded Cumulative CES district-opinion extract when network access and the optional Feather parser dependency are available.
