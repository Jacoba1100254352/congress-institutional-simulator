...
The model compares legislative institutional designs under shared generated political worlds. Its purpose is to stress-test mechanisms that alter productivity, compromise-like moderation, representative responsiveness, public-benefit alignment, minority protection, and resistance to capture. It is not a forecast of a specific Congress. It is a mechanism-comparison simulator: the same generated legislators and bills are routed through many institutional processes so differences can be attributed to procedural rules and incentive structures.
...
\begin{table*}
  ...
  \begin{tabular}{lllll}
    ...
    Sensitivities & \(c_i,\pi_i,\kappa_i,\lambda_i,r_i\) & scenario center plus bounded Gaussian noise & yes & compromise, party, constituency, lobby, reputation weights \\
...
\begin{table*}
  ...
  \begin{tabular}{llll}
    ...
    Compromise & compromise preference times \(0.45\) moderation \(+0.35\) public legitimacy \(+0.20\) incrementalism & 0.75 & 0.82 \\
...
\begin{figure}
  ...
  \caption{Appendix rising-contention stress paths from the main comparison campaign. The index is \(0.50\cdot\)gridlock \(+0.30\cdot(1-\)compromise\()+0.20\cdot\)low-public-support enactment. Lower is better.}
...
\begin{figure}
  ...
  \caption{Chamber-structure family champions from the supplemental chamber campaign, zoomed to the observed champion range so clustered family labels remain readable. The axes keep the paper's central productivity/compromise comparison visible; malapportionment remains in Table~\ref{tab:chamber-family-champions} because the champion set has little variation on that metric.}
  \label{fig:chamber-productivity-compromise}
  \Description{Scatter plot of chamber family champion productivity and compromise, with the conventional benchmark marked by a distinct label and marker.}
\end{figure}
...
\begin{itemize}
  ...
  \item \texttt{make empirical-bridge}: map raw summaries to simulator proxies for empirical flow sanity checks.
