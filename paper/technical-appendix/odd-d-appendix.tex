...
The paired govinfo lifecycle censuses cover every H.R. and S. record in the pinned 117th- and 118th-Congress BILLSTATUS bulk archives: respectively 15,066 bills and 72,047 direct actions, then 16,213 bills and 75,239 direct actions. Each normalized row stores byte-level source-record and canonical action hashes, archive provenance, stage evidence, and explicitly preserved source-date anomalies (five in the 117th and twenty in the 118th). Operational classifiers distinguish ordered reported from a report action, use an ordered-reported/reported/discharged union for committee advancement, and require substantive action for floor consideration. The GPO guide cautions that action types are processing categories and provides no complete authoritative code list, so these stages are reproducible research classifications rather than official legal-status determinations.

Full 118th processing exposed a context-dependent presidential action code: \texttt{E30000} labels signatures in most records but the veto of S.4199 in the Senate-origin stream. Classifier v2 therefore removes \texttt{E30000} from code-only enactment and requires positive signature/enactment text or an unambiguous law record or code. Applied symmetrically, the correction changes no 117th aggregate count and classifies the 118th as 270 presented bills, 269 enactments, and one vetoed non-enactment. Three additional 118th bills have separate House and Senate passage evidence but lack evidence of agreement on identical text, so the conservative completed-passage flag remains false.
...
The complete 118th census is then used without refitting. Its committee-advance, substantive-floor, and enactment rates are 0.113674, 0.059027, and 0.016592. Against the frozen simulator means, absolute errors are 0.007396, 0.002070, and 0.010825 under prespecified tolerances of 0.020, 0.015, and 0.010. Thus two of three point-error checks pass, while enactment misses by 0.000825. All three observed rates remain inside the broader 117th-derived benchmark ranges, but that result does not override the stricter transport miss. This is aggregate temporal evidence for one later Congress, not causal or mechanism-level validation.
...
\begin{itemize}
  ...
  \item \texttt{make legislative-lifecycle-temporal-replication}: apply the frozen selected threshold to the 118th-Congress rates without refitting.
  \item \texttt{make govinfo-bill-census-check}: verify both censuses, archive pins, classifier edge cases, cross-source agreement, calibration linkage, and the preserved transport outcome.
  ...
  \item \texttt{make build-govinfo-bill-census-118-raw}: rebuild the pinned 118th-Congress H.R./S. temporal cohort from the official bulk archives.
