...
The executable checker runs conventional scenarios and writes CSV and Markdown pass/fail reports under \texttt{reports/}. The pass is an empirical screen, not an automatic parameter fitter. Its benchmark ranges are intentionally broad and several are abstract proxies, so passing the screen should be read as minimum plausibility rather than validation. The empirical comparison checks include documented held-out and flow/proxy samples: Congress.gov bill progression with a bounded govinfo BILLSTATUS cross-check, a bounded sponsor aggregate with sponsor-bill metadata matches by Bioguide ID, Voteview roll-call rows carrying bounded member metadata and bill-number linkage, Senate LDA lobbying disclosures with bounded issue-policy context, bounded OpenFEC campaign-finance rows with FEC recipient metadata, broad transaction-label issue context, matched-candidate Voteview member context, House-candidate district context for small subsets, public FEC/OpenFEC target-scope review for queued campaign-finance rows, cached committee/floor bill-action context and official source-acquisition targets for queued finance/lobbying rows, a bounded Cumulative CES district-opinion aggregate \citep{CumulativeCES2025} plus Congress.gov sponsor-district bill metadata and bounded bill policy-area context, a bounded QoG/OWID/V-Dem comparative-institution profile with simulator scenario-family metadata anchors \citep{BormannGolder2022,HeniszPOLCON2017,OWIDVDEMJudicial2026,OWIDVDEMLegislative2026}, SCDB merits review with bounded U.S.C.-section authority-overlap metadata, Federal Register final-rule effective dates with Federal Register document/docket metadata, a bounded public-law authority-search cache, and a bounded proposed-history/comment-portal/timing cache, Congress.gov public-law revision text with bounded bill/action metadata, and Congress.gov-derived topic and committee summaries. These samples support nine narrow held-out source-family checks for bill attrition and floor load, roll-call coalition size, sponsor concentration, campaign-finance concentration and outside spending, district public-will proxies, merits-case invalidation, final-to-effective delay, public-law revision text flags, and comparative bicameral context. The govinfo layer is an independent metadata cross-check for the bounded bill sample; sponsor-bill metadata, district public-opinion policy-area context, comparative scenario-family metadata anchors, LDA issue-policy context, FEC issue-sector and target-scope context, cached committee/floor bill-action context, finance/lobbying source-acquisition targets, Federal Register authority text, Federal Register proposed-history/comment-portal/timing metadata, SCDB U.S.C.-section overlap metadata, topic-throughput, and committee summaries remain flow or calibration proxy evidence. A separate linkage audit reports all thirteen source families as linked, metadata-linked, or partially linked, and the linkage roadmap records required join keys and acceptance gates for the remaining non-fully-linked families. Optional API fixture samples are stored separately from raw empirical inputs; they test adapter shapes but are not used as validation evidence.
