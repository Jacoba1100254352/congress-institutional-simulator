...
def write_timeline_contention(
    ...
    scenarios = [
        ("presidential-veto", "Veto", "black!55", 67.0),
        (CURRENT_SYSTEM_KEY, "Current", "red", 61.0),
        ("simple-majority", "Simple maj.", "black", 55.0),
        ("default-pass-alternatives-pairwise", "Pairwise", "black!80", 48.5),
        ("default-pass", "Default", "black!30", 43.0),
        ("default-pass-challenge", "Challenge", "black!70", 37.5),
        ("default-pass-multiround-mediation-challenge", "Med.+chal.", "black!45", 32.0),
    ]
    ...
    for scenario_key, label, color, label_y in scenarios:
        ...
        leader_color = "red!70" if scenario_key == CURRENT_SYSTEM_KEY else "black!35"
        leader_mid_x = (final_x + label_x - 1.6) / 2.0
        leader_mid_y = (final_y + label_y) / 2.0
        lines.extend([
            "\\linethickness{0.12mm}",
            "{\\color{" + leader_color + "}"
            + f"\\qbezier[10]({fmt(final_x + 1.6)},{fmt(final_y)})"
            + f"({fmt(leader_mid_x)},{fmt(leader_mid_y)})"
            + f"({fmt(label_x - 1.6)},{fmt(label_y)})"
            + "}",
        ])
        lines.append(
    ...
    lines.extend([
        ...
        f"\\put({fmt(left + width / 2.0)},{fmt(78.5)}){{\\makebox(0,0){{Contention = 0.50 gridlock + 0.30 compromise loss + 0.20 low-support}}}}",
        "\\end{picture}",
        "\\endgroup",
        "",
    ])
