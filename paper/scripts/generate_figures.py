...
def sorted_main_table_scenarios(rows: list[dict[str, str]]) -> list[tuple[str, str]]:
    present_keys = {row["scenarioKey"] for row in rows}

    def directional_average(scenario: tuple[str, str]) -> tuple[float, str]:
        scenario_key, label = scenario
        values = scenario_case_values(rows, scenario_key, "directionalScore")
    ...
    return sorted(scenarios, key=directional_average, reverse=True)
...
def write_directional_scoreboard(
    ...
    lines.extend([
        ...
        f"\\put({fmt(left_matrix + 2.5 * col_width)},{fmt(1.4)}){{\\makebox(0,0){{Rows sorted by overall directional score; green is favorable and red is unfavorable.}}}}",
        "\\end{picture}",
        "\\endgroup",
        "",
    ])
...
def write_timeline_contention(
    ...
    left, bottom, width, height = 16.0, 14.0, 100.0, 58.0
    data_padding = 14.0
    data_left = left + data_padding
    data_width = width - 2.0 * data_padding
    lines = [
        "% Auto-generated by paper/scripts/generate_figures.py",
        "\\begingroup",
        "\\setlength{\\unitlength}{1mm}",
    ...
    for index, case_key in enumerate(case_keys):
        x = data_left + (index / max(1, len(case_keys) - 1)) * data_width
        label = f"E{index + 1}"
        lines.extend([
    ...
    lines.extend([
        ...
        f"\\put({fmt(left + width / 2.0)},{fmt(bottom + height + 2.4)}){{\\makebox(0,0){{Contention index (lower is better)}}}}",
