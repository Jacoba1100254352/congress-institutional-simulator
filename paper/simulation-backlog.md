...
- empirical priority: small adapter samples remain fixtures under `data/validation/fixtures/`; the next serious paper-strengthening step is replacing the remaining missing inputs with curated raw extracts under `data/validation/raw/`, with documented source transformations and tighter target ranges. The current sponsor and committee raw summaries are usable bridge signals but remain sampled and action-derived rather than full committee-calendar or member-history datasets
