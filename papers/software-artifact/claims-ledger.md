...
| S4 | The project includes empirical and validation-related scripts. | `scripts/validation`; `reports/empirical-*`; three GovInfo census reports; `reports/legislative-lifecycle-calibration.*`; `reports/legislative-lifecycle-temporal-replication.*`; `reports/legislative-executive-action-diagnostic.*`. | These scripts perform source audits, proxy checks, two narrow external-Congress no-refit aggregate-flow tests, and a descriptive executive-action boundary diagnostic, not general model validation. | Data-source and claim-boundary documentation if discussed. | Supported with boundary language. |
