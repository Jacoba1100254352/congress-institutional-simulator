...
Go for expansion beyond the fixed-specification A1-A9 seed panels. Conditional go for a full draft only after the remaining mechanism, parameter, correction, and validation gates are satisfied.
...
|---|---|---|---|
| Current repo has ablation, manipulation-stress, bounded A1-A9 explicit adversary pilots, and fixed-specification seed replication. | `reports/simulation-ablation-analysis.csv`, `reports/simulation-manipulation-stress.csv`, A1-A9 adversarial summaries/traces, and the A1-A8 and A9 replication summaries. | Current attacks cover bounded mechanism paths and are not externally validated. | Usable as bounded synthetic evidence only. |
...
| A robustness paper can stand apart from the ACM framework paper. | Failure modes are a distinct technical object; A1-A9 provide bounded explicit examples and scalable checkpointed seed-replication paths. | Requires broader mechanisms and parameters, alternative A9 specifications, correction evidence, and validation before drafting. | Conditional. |
