...
| The empirical layer provides flow sanity checks, two narrow external-Congress temporal transport tests, and an 11-Congress executive-action boundary diagnostic. | `reports/calibration-baseline.*`, `reports/empirical-bridge.*`, three GovInfo lifecycle census reports, the compact executive panel, `reports/legislative-lifecycle-temporal-replication.*`, and `reports/legislative-executive-action-diagnostic.*`. | Five of six cohort-metric cells pass; 118th-Congress enactment misses, the veto-rate diagnostic identifies a large executive-choice mismatch, and none of this validates welfare, harm, capture, support, revision moderation, or causal mechanisms. | Keep with explicit miss and stress-mechanism boundary. |
