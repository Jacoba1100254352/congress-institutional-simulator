...
Allowed language now: "the A1-A9 catalog defines planned adversary tests, and the A1 clone/decoy through A7 administrative-overload pilots provide bounded synthetic evidence for seven attack families."
...
|---|---|---|---|---|---|---|
| C1 | The repo can support a robustness breakout distinct from the ACM framework paper. | The simulator has relevant mechanism surfaces, pilot reports, an explicit A1-A9 adversary catalog under `src/main/java/congresssim/institution/adversary/`, and executable A1-A7 stress runners. | Needs executable A8-A9 adversary actions and reports plus broader A1-A7 coverage. | External adversarial behavior is not validated. | The breakout is feasible as a planned extension. | Planning-supported. |
...
| C5 | Public-input systems may be vulnerable to astroturf, noise, or panel manipulation. | Current aggregate pilot includes citizen-panel manipulation and astroturf objection pressure. `reports/adversarial-stress-a3-summary.md` now adds a bounded A3 budget/information pilot with attack success rates, false-positive blockage, false-negative clearance, low-support enactment, administrative-burden deltas, and 1,800 same-generated-world per-bill trace rows. | Needs petition and challenge-token coverage, recovery/correction behavior, seed sensitivity, and separation from A8 public-support distortion. | Need public-comment, panel, or objection-process data before external claims. | In the current A3 synthetic pilot, public-input manipulation can be reported only as a bounded public-objection plus citizen-panel stress result. | Conditional. |
...
| C10 | Average-case performance can hide adversarial failure. | The ACM framework, pilot reports, aggregate trace index, and A1-A7 stress summaries separate mean, median, worst-case, and per-bill trace evidence for seven attack families. | Need the same reporting for A8-A9, broader mechanisms, and multi-seed runs. | No external adversarial benchmark. | The current A1-A7 pilots can be used as bounded examples of why average stress summaries are not enough. | Conditional. |
