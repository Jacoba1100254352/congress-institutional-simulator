...
Allowed language now: "the A1-A9 catalog and bounded executable pilots provide synthetic first-wave evidence for nine attack families, including fixed-budget mixed portfolios compared with single-attack controls."
...
|---|---|---|---|---|---|---|
| C1 | The repo can support a robustness breakout distinct from the ACM framework paper. | The simulator has relevant mechanism surfaces, pilot reports, an explicit A1-A9 adversary catalog under `src/main/java/congresssim/institution/adversary/`, and bounded executable stress runners for all nine entries. | Needs broader mechanism coverage, multi-seed adversarial replication, stronger correction tests, and validation. | External adversarial behavior is not validated. | The breakout is feasible and now has complete bounded first-wave implementation. | Planning-supported. |
...
| C10 | Average-case performance can hide adversarial failure. | The ACM framework, pilot reports, aggregate trace index, and A1-A9 stress summaries separate mean, median, worst-case, and per-bill trace evidence for nine attack families. | Need broader mechanisms and multi-seed runs. | No external adversarial benchmark. | The current A1-A9 pilots can be used as bounded examples of why average stress summaries are not enough. | Conditional. |
...
| C13 | Mixed adversary portfolios can expose interactions that single-attack probes miss. | `reports/adversarial-stress-a9-summary.md` reports 18 fixed-budget portfolio/information cells and `reports/adversarial-failure-traces-a9.jsonl` provides 5,400 paired traces with full-budget and allocated-component single controls. Under the canonical synthetic seed, 99 traces meet the strict mixed-only condition: the mixed portfolio causes an adverse failure while no full-budget constituent control does. | Needs independent seeds, broader mechanism variants, alternative interaction specifications, and substantive correction/replay. | No empirical benchmark for coordinated multi-actor attack frequency or resource conversion. | In the bounded A9 synthetic pilot, mixed-only failures occur under the named portfolio, budget, and process assumptions. | Conditional. |
