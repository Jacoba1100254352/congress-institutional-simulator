...
CATALOG_DIR = ROOT / "src/main/java/congresssim/simulation/catalog"
...
def historical_keys() -> list[str]:
    text = "\n".join(
        path.read_text()
        for path in sorted(CATALOG_DIR.glob("*ScenarioFamily.java"))
    )
    return re.findall(r'new ScenarioEntry\("([^"]+)"', text)


def breadth_keys() -> list[str]:
    text = (CATALOG_DIR / "ScenarioCatalog.java").read_text()
...
def family(key: str) -> str:
    ...
    if "constitutional-court" in key:
        return "Constitutional court architecture"
    if "district-population" in key:
        return "Constituent population"
    if "influence-system" in key:
        return "Influence/campaign finance"
    if "omnibus" in key:
    ...
    if "adaptive" in key or "risk" in key or "norm" in key or "strategy" in key or "learning" in key or "deterioration" in key:
