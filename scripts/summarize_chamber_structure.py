...
def write_paper_table(champs: list[dict[str, str]]) -> None:
    ...
    PAPER_TABLE.write_text("% Auto-generated by scripts/summarize_chamber_structure.py\n" + "\n".join(rows) + "\n")
...
def write_paper_figure(champs: list[dict[str, str]]) -> None:
    ...
    bottom = 14.0
    lines = [
        "% Auto-generated by scripts/summarize_chamber_structure.py",
        r"\begingroup",
    ...
    for index, row in enumerate(champs):
        ...
        intensity = int(round(15 + 70 * max(0.0, min(1.0, f(row, "malapportionmentIndex")))))
        ...
        lines.append(fr"{{\color{{black!35}}\qbezier[6]({x:.1f},{y:.1f})({x + dx / 2:.1f},{y + dy / 2:.1f})({x + dx:.1f},{y + dy:.1f})}}")
