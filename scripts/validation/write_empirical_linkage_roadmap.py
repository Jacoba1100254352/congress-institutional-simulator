...
ROADMAP: dict[str, dict[str, str]] = {
    ...
    "District public opinion and affected groups": {
        "blockingGap": "CES district rows can now join to House-sponsored public-law bill metadata and bounded bill policy-area context by sponsor district when the linkage caches are present, the bill-topic readiness queue converts those proxy-only public-law rows into source-review packets, the source-packet report names survey, MRP/small-area, and affected-population acquisition targets, the Census denominator report joins queued sponsor districts to official TIGERweb 2020 population and housing-unit denominators, the ACS context report joins queued sponsor districts to official 2017-2021 ACS 5-year broad demographic, economic, citizenship, language, disability, internet, and veteran context, the survey-source crosswalk maps the queued packets to official survey/source families and candidate item-search terms, the survey item proxy-review layer records exact current CES proxy variables for those packets, the CES policy-preference candidate-item review maps 18 / 22 queued packet rows to exact Cumulative CES Policy Preferences variable IDs while leaving 4 packet rows without a candidate item in that source, the response-distribution review attaches unweighted raw response-code distributions for those candidate variables where present, and the codebook response-direction review attaches official guide response labels or endpoint labels for those candidate variables where present. They still do not measure issue-specific bill support, source-reviewed exact bill-topic survey item wording, bill-text-aligned support/opposition direction, MRP/small-area opinion, bill-text-specific affected-population definitions, or issue-specific affected-group support and harm.",
        "requiredJoinKeys": "district_id,issue_topic,policy_area,bill_id,affected_group",
        "targetSourceFamilies": "Cumulative CES/CCES or ANES item-level public opinion; Census TIGERweb; Census/ACS; congressional district crosswalks; affected-group data",
        "minimumViableDataset": "The current sponsor-district bill metadata, bounded bill policy-area context, bill-topic readiness queue, source-acquisition packets, Census population/housing denominators, broad ACS district context, official survey-source crosswalk, exact current CES proxy-variable review, official CES policy-preference candidate-item review, unweighted raw response-code distribution review, and official guide codebook response-direction review plus source-reviewed exact bill-topic survey wording, bill-text direction alignment, respondent geography, MRP or small-area estimates where needed, and bill-text-specific affected-population joins.",
        "acceptanceGate": "At least one district-opinion row joins beyond sponsor-district bill policy-area context to issue-specific bill support or issue-specific affected-group data, with affected-population fields separated from general support.",
        "futureTarget": "Use reports/district-public-opinion-source-packets.csv, reports/district-public-opinion-census-denominators.csv, reports/district-public-opinion-acs-context.csv, reports/district-public-opinion-survey-source-crosswalk.csv, reports/district-public-opinion-survey-item-proxy-review.csv, reports/district-public-opinion-ces-policy-item-candidate-review.csv, reports/district-public-opinion-ces-policy-item-response-distribution-review.csv, and reports/district-public-opinion-ces-policy-item-codebook-direction-review.csv as the sponsor-district frame for separating existing CES proxy variables, broad policy-preference candidate items, raw response-code distributions, and guide codebook item-direction labels from missing exact bill-topic support items, bill-text direction alignment, MRP or small-area estimates, and bill-text-specific affected-population, support, and harm sources.",
        "claimUpgradeBoundary": "Supports district-level survey aggregation, bounded sponsor-district bill policy-area context, bill-topic readiness queueing, source-acquisition packet planning, official population/housing denominator context, broad ACS district context, survey-source crosswalk planning, exact current CES proxy-variable review, official broad CES policy-preference candidate-item IDs, unweighted raw response-code distributions, and official guide codebook item-direction labels only; this is not bill-text-aligned support/opposition, exact bill-topic support, MRP, bill-text-specific affected-population detail, issue-specific affected-group harm, public-benefit validation, or model validation.",
    },
